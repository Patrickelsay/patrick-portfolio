...
export type Block =
  ...
  | { kind: 'media'; media: string; caption?: string; full?: boolean }
  | { kind: 'grid'; items: { media: string; caption?: string }[]; cols?: 2 | 3 | 4 }
  | { kind: 'memo'; text: string }
  | { kind: 'list'; title?: string; items: string[]; ordered?: boolean }
  | { kind: 'meta'; items: { label: string; value: string }[] }
  | { kind: 'sequence'; media: string; caption?: string }
  | { kind: 'stats'; items: { value: string; label: string }[] }
  | {
      kind: 'process'
      title?: string
      intro?: string
      items: { media: string; caption?: string }[]
    }
...
export interface Project {
  ...
  tier: 1 | 2 | 3
  /** media id for the case-study hero (dark room) */
  hero: string
  /** media id for grid cards; defaults to hero when omitted */
  card?: string
  /** optional video id that plays on card hover */
  cardVideo?: string
  summary: string
